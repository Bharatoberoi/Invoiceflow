...
def create_or_load_vectorstore(documents=None, name="docs"):
    ...
    # If no documents, create an empty collection
    if not documents:
        print("[INFO] No documents provided. Creating empty Chroma collection...")
        return Chroma(
    ...
    # Otherwise, create it
    print("[INFO] Creating ChromaDB and embedding documents...")
        ...
        for tool_call in response.tool_calls:
            ...
            try:
                if tool_name == "search_orders":
                    # Extract query string - handle nested structures
                    query = tool_args
                    if isinstance(tool_args, dict):
                        if 'query' in tool_args:
                            query = tool_args['query']
                        elif 'content' in tool_args:
                            query = tool_args['content']
                    result = search_orders.invoke({"query": str(query)})
                elif tool_name == "get_order_status":
                    order_id = tool_args
                    if isinstance(tool_args, dict):
                        order_id = tool_args.get('order_id', tool_args)
    ...
    if response.tool_calls:
        ...
        for tool_call in response.tool_calls:
            ...
            if tool_name == "search_invoices":           
                result = search_invoices.invoke(tool_args.get('query') if isinstance(tool_args, dict) else tool_args)
            else:
                result = f"Unknown tool: {tool_name}"
